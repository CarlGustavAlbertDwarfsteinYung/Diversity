    ...
    protected boolean build(World world, Random random, StructureBoundingBox structureBoundingBox) {
        if (!this.func_74935_a(world, structureBoundingBox, 0)) {
            ...
            if (this.caveH != -100) {
                ...
                for(index = 1; index < this.sphereCenter.size(); ++index) {
                    if (structureBoundingBox.intersectsWith(((Point)this.sphereCenter.get(index)).x, ((Point)this.sphereCenter.get(index)).z, ((Point)this.sphereCenter.get(index)).x, ((Point)this.sphereCenter.get(index)).z)) {
                        EntityLiving entitySpider = new EntitySpider(world);
                        entitySpider.setLocationAndAngles((double)((Point)this.sphereCenter.get(index)).x + 0.5D, (double)((Point)this.sphereCenter.get(index)).y + (double)this.caveH, (double)((Point)this.sphereCenter.get(index)).z + 0.5D, 0.0F, 0.0F);
            ...
            while(true) {
                ...
                while(true) {
                    while(true) {
                        ...
                        if (this.blocks.containsKey(x, y - 1, z) && this.blocks.get(x, y - 1, z).equals(EnumCubeType.GROUND) && world.getTopSolidOrLiquidBlock(x, z) - (y + this.caveH) > 3 && random.nextInt(6) == 0) {
                            ...
                            world.setBlock(x, y + this.caveH, z, Blocks.web, 1, 0);
                        } else {
                    ...
                    if (this.blocks.containsKey(x, y, z)) {
                        if (world.getTopSolidOrLiquidBlock(x, z) > y + this.caveH) {
                            if (this.blocks.get(x, y, z).equals(EnumCubeType.ROOF)) {
                                world.setBlock(x, y + this.caveH, z, Blocks.air);
                            } else if (!this.blocks.get(x, y, z).equals(EnumCubeType.WALL)) {
                                if (this.blocks.get(x, y, z).equals(EnumCubeType.GROUND)) {
                                    world.setBlock(x, y + this.caveH, z, Blocks.dirt, 1, 3);
                                } else if (this.blocks.get(x, y, z).equals(EnumCubeType.UNDERGROUND)) {
                                    world.setBlock(x, y + this.caveH, z, Blocks.dirt);
                                }
                            }
                        }
